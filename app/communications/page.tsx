...
export default function CommunicationsPage() {
  return (
    <main className="min-h-screen bg-white text-slate-950">
      <section className="border-b border-slate-200 bg-[linear-gradient(180deg,#ffffff,#f8fafc)]">
        <div className="mx-auto max-w-7xl px-6 py-16 lg:px-10 lg:py-20">
          <div className="grid gap-8 lg:grid-cols-[minmax(0,1fr)_24rem] lg:items-end">
            <div className="max-w-4xl space-y-6">
              <div className="inline-flex rounded-lg border border-slate-300 bg-white px-4 py-1 text-xs font-semibold uppercase tracking-wide text-slate-600">
            ...
            <div className="rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
              ...
              <p className="mt-3 text-3xl font-semibold text-slate-950">
                {enterprisePhoneLines[0]?.display ?? "HQ line pending"}
              </p>
              <p className="mt-2 text-sm leading-6 text-slate-600">
                Voice-first HQ routing with brand-owned messaging lanes for consent, reputation,
          ...
          <div className="mt-8 flex flex-wrap gap-3">
            <Link
              ...
              className="inline-flex items-center rounded-lg border border-slate-900 bg-slate-950 px-5 py-3 text-sm font-semibold text-white transition-colors hover:bg-slate-800"
            >
              Open network console
            </Link>
            <Link
              href="/teaming-events"
              className="inline-flex items-center rounded-lg border border-slate-300 bg-white px-5 py-3 text-sm font-semibold text-slate-900 transition-colors hover:border-slate-950"
            >
              View teaming events
            </Link>
          </div>
      ...
      <CommunicationsConsole
        profiles={brandCommunicationProfiles}
        policies={enterpriseChannelPolicies}
        phoneLines={enterprisePhoneLines}
        ivrTree={hqIvrTree}
        kpis={enterpriseCommunicationKpis}
      />
          ...
          <Panel title="Current Number Strategy" eyebrow="Immediate move">
            <div className="space-y-4">
              {enterprisePhoneLines.map((line) => (
                <div key={line.e164} className="rounded-lg border border-slate-200 bg-slate-50 p-4">
                  <div className="flex flex-wrap items-center justify-between gap-3">
                    ...
                    <span className={`rounded-lg border px-3 py-1 text-xs font-semibold uppercase tracking-wide ${statusClasses(line.status)}`}>
        ...
        <Panel title="HQ IVR Tree" eyebrow="Tolani Corp switchboard">
          <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
            {hqIvrTree.map((branch) => (
              <div key={branch.digit} className="rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
                <p className="text-xs font-semibold uppercase tracking-wide text-slate-500">
                  Press {branch.digit}
                </p>
        ...
        <Panel title="Brand Routing Profiles" eyebrow="Communication ownership by brand">
          <div className="grid gap-5 lg:grid-cols-2">
            {brandCommunicationProfiles.map((brand) => (
              <div key={brand.key} className="rounded-lg border border-slate-200 bg-slate-50 p-6">
                <div className="flex flex-wrap items-start justify-between gap-3">
                  <div>
                    <h3 className="text-xl font-bold text-slate-900">{brand.name}</h3>
                    <p className="text-sm text-slate-500">{brand.domain}</p>
                  </div>
                  <span className="rounded-lg border border-slate-300 bg-white px-3 py-1 text-xs font-semibold uppercase tracking-wide text-slate-600">
                    {brand.aiAgentLabel}
                  </span>
                </div>
          ...
          <Panel title="Channel Policies" eyebrow="How each lane should behave">
            <div className="space-y-4">
              {enterpriseChannelPolicies.map((policy) => (
                <div key={policy.key} className="rounded-lg border border-slate-200 bg-slate-50 p-4">
                  ...
                  <div className="mt-3 flex flex-wrap gap-2 text-xs font-semibold uppercase tracking-wide">
                    <span className="rounded-lg border border-slate-300 bg-white px-3 py-1 text-slate-600">
          ...
          <Panel title="Rollout Plan" eyebrow="Optimized implementation sequence">
            <div className="space-y-4">
              {rolloutPlan.map((phase) => (
                <div key={phase.phase} className="rounded-lg border border-slate-200 bg-slate-50 p-6">
                  ...
                  <ul className="mt-4 space-y-2 text-sm leading-7 text-slate-600">
                    {phase.actions.map((action) => (
                      <li key={action} className="rounded-lg border border-slate-200 bg-white px-4 py-3">
                        {action}
                      </li>
                    ))}
                  </ul>
        ...
        <Panel title="Operating Metrics" eyebrow="What management should track">
          <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
            {enterpriseCommunicationKpis.map((kpi) => (
              <div key={kpi} className="rounded-lg border border-slate-200 bg-slate-50 px-4 py-4 text-sm font-medium text-slate-700">
                {kpi}
              </div>
            ))}
          </div>
  ...
  return (
    <div className={className}>
      ...
      <ul className="mt-3 space-y-2">
        {items.map((item) => (
          <li key={item} className="rounded-lg border border-slate-200 bg-white px-4 py-3 text-sm leading-6 text-slate-600">
            {item}
          </li>
        ))}
      </ul>
...
function SurfaceLink({
  ...
  return (
    <Link
      href={href}
      className="block rounded-lg border border-slate-200 bg-slate-50 px-5 py-5 transition-colors hover:border-slate-950 hover:bg-white"
    >
